...
class Credential(models.Model):
    ...
    proxyport = models.PositiveIntegerField(blank=True, null=True, verbose_name=_('Proxy port'),default='0')
...
class ServerInfor(models.Model):
    ...
    def gethostname(self):
        return slugify('{0} {1} {2} {3}'.format(self.name, self.ip, self.hostname, ''.join(
...
class Log(models.Model):
    server = models.ForeignKey(ServerInfor, verbose_name=_('Server'),on_delete=models.CASCADE)
    ...
    user = models.ForeignKey(User, verbose_name=_('User'), related_name='user',on_delete=models.CASCADE)
    ...
    gucamole_client_id = models.CharField(max_length=100, verbose_name=_('Gucamole channel name'), blank=True,
                                          editable=False)
...
class CommandLog(models.Model):
    log = models.ForeignKey(Log, verbose_name=_('Log'),on_delete=models.CASCADE)
...
class Menu(models.Model):
    name = models.CharField(max_length=32,unique=True,verbose_name=u'菜单名')
    parent = models.ForeignKey('self',verbose_name=u'父菜单',null=True,blank=True,
                               default='0',help_text=u'如果添加的是子菜单，请选择父菜单',on_delete=models.CASCADE)
    show = models.BooleanField(verbose_name=u'是否显示',default=False,help_text=u'菜单是否显示，默认不显示')

    url = models.CharField(max_length=300,verbose_name=u'菜单url',null=True,blank=True,default='javascript:void(0)')
    priority = models.IntegerField(verbose_name=u"显示优先级",null=True,blank=True,default=-1,help_text=u"菜单显示顺序，优先级越大显示越靠前")
    permission_id = models.IntegerField(verbose_name=u"权限编码",help_text=u'给菜单设置一个编码，用于权限控制',
